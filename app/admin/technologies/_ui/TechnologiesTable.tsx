...
interface TechnologiesTableProps {
    initialTechnologies: TechnologyItem[];
    errorMessage?: string;
}

// Función para transformar el nombre en slug amigable
function createTechnologySlug(name: string): string {
    return name
        .toLowerCase()
        .trim()
        .normalize('NFD')
...
export default function TechnologiesTable({
    ...
    const [technologies, setTechnologies] = useState<TechnologyItem[]>(initialTechnologies);
    ...
    // Mantener sincronizado si initialTechnologies cambia
    useEffect(() => {
        setTechnologies(initialTechnologies);
    }, [initialTechnologies]);

    // Función para copiar texto al portapapeles con feedback háptico
    const handleCopy = async (text: string, label: string = 'Texto', idFeedback?: string | number) => {
        try {
    ...
    const handleRefresh = () => {
        ...
        startTransition(async () => {
            try {
                const res = await getAllTechnologiesAction();
                if (res.ok && res.data) {
                    setTechnologies(res.data);
                    toast.add({
                        title: 'Tecnologías sincronizadas',
                        description: `Se han actualizado ${res.data.length} tecnologías con éxito.`,
                        type: 'success',
                    });
                } else {
                    toast.add({
                        title: 'Error al sincronizar',
                        description: res.msg || 'No se pudieron cargar las tecnologías del servidor.',
                        type: 'error',
                    });
                }
            } catch {
    ...
    const columns = useMemo<ColumnDef<TechnologyItem>[]>(
        () => [
            {
                id: 'select',
                header: ({ table }) => (
                    <div className="flex items-center justify-center pl-2">
                        <Checkbox
                            ...
                            aria-label="Seleccionar todas las tecnologías visibles"
                        />
                    </div>
                ),
                cell: ({ row }) => (
                    <div className="flex items-center justify-center pl-2">
                        <Checkbox
                            ...
                            aria-label={`Seleccionar tecnología ${row.original.name}`}
                        />
                    </div>
                ),
                enableSorting: false,
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    ...
                    const slug = createTechnologySlug(name);
                    return (
                        <div className="flex items-center gap-3.5 py-1.5">
                            <div className="relative flex size-10 shrink-0 items-center justify-center rounded-xl border border-purple-500/25 bg-gradient-to-br from-purple-500/15 via-purple-600/10 to-fuchsia-500/10 text-purple-600 shadow-xs dark:text-purple-400">
                                <HugeiconsIcon icon={SourceCodeIcon} strokeWidth={2} className="size-4.5" />
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <button
                            type="button"
                            onClick={() => handleCopy(String(id), 'ID de tecnología', id)}
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <div className="flex items-center justify-end gap-2 pr-2">
                            <Link
                                href={`/admin/technologies/new?id=${technology.id}`}
    ...
    const handleCopySelectedIds = () => {
        ...
        handleCopy(ids, `${selectedRows.length} IDs de tecnologías`);
    };
            ...
            >
                <div className="space-y-2">
                    ...
                    <p className="max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-base">
                        Administra las tecnologías, frameworks y herramientas asociadas a los cursos
                        y rutas de aprendizaje de Devtalles.
                    </p>
                </div>
            ...
            >
                ...
                <div className="relative flex flex-col overflow-hidden rounded-[calc(2rem-0.625rem)] border border-border/60 bg-card/95 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] backdrop-blur-xl dark:border-white/[0.08] dark:bg-zinc-950/80">
                    ...
                    <div className="flex flex-col gap-4 border-b border-border/50 p-4 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-5">
                        {/* Buscador reactivo por Nombre de Tecnología */}
                        <div className="relative flex-1 sm:max-w-md">
                            ...
                            <input
                                ...
                                placeholder="Filtrar por nombre de tecnología..."
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div className="inline-flex h-10 items-center gap-1.5 rounded-xl border border-border/50 bg-muted/30 px-3 text-xs font-semibold text-muted-foreground">
                                ...
                                <span>{table.getFilteredRowModel().rows.length} tecnologías</span>
                            </div>
                        </div>
                    </div>
                                            ...
                                            <div className="flex flex-col items-center justify-center gap-3 py-10">
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="max-w-sm text-xs text-muted-foreground">
                                                        {searchFilterValue
                                                            ? `No existe ninguna tecnología que coincida con "${searchFilterValue}". Intenta con otro término.`
                                                            : 'Comienza creando la primera tecnología para vincularla a los cursos de la plataforma.'}
                                                    </p>
                                                </div>
                                                {searchFilterValue ? (
                                                    <button
                                                    ...
                                                    >
                                                        ...
                                                        <span>Crear primera tecnología</span>
                                                    </Link>
                                                )}
                                            </div>
                                        </TableCell>
